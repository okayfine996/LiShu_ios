...
private func makeStatisticsPreviewContainer() -> ModelContainer? {
    ...
    let e2 = Event(name: "添丁满月", type: .birth, date: cal.date(from: DateComponents(year: thisYear, month: 2, day: 10)).unwrappedOrNow)
    let e3 = Event(name: "生日祝寿", type: .birthday, date: cal.date(from: DateComponents(year: thisYear, month: 5, day: 20)).unwrappedOrNow)
    let e4 = Event(name: "乔迁新居", type: .property, date: cal.date(from: DateComponents(year: thisYear, month: 8, day: 5)).unwrappedOrNow)
    ...
    let records: [Record] = [
        ...
        Record.makeMonetaryRecord(
            ...
            date: cal.date(from: DateComponents(year: thisYear, month: 1, day: 16)).unwrappedOrNow
        ),
        Record.makeMonetaryRecord(
            contact: c2,
            event: e3,
        ...
        Record.makeMonetaryRecord(
            ...
            direction: .received,
            paymentMethod: .wechat,
            date: cal.date(from: DateComponents(year: thisYear, month: 8, day: 5)).unwrappedOrNow
        ),
        Record.makeMonetaryRecord(
            contact: c3,
            event: e4,
