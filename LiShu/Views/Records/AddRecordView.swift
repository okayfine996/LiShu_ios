...
private func makeAddRecordPreviewContainer() -> ModelContainer? {
    ...
    let e1 = Event(name: "表哥的婚礼", type: .wedding, date: cal.date(byAdding: .day, value: 5, to: now).unwrappedOrNow, location: "家乡酒店")
    let e2 = Event(name: "小李30岁生日", type: .birthday, date: cal.date(byAdding: .day, value: 12, to: now).unwrappedOrNow, location: "上海")
    let e3 = Event(name: "硕士毕业典礼", type: .education, date: cal.date(byAdding: .day, value: 20, to: now).unwrappedOrNow, location: "广州")
    let e4 = Event(name: "春节拜年", type: .festival, date: cal.date(byAdding: .month, value: -1, to: now).unwrappedOrNow, location: "老家")
    let e5 = Event(name: "王总乔迁", type: .property, date: cal.date(byAdding: .day, value: -5, to: now).unwrappedOrNow, location: "深圳")
    let e6 = Event(name: "爷爷丧事", type: .funeral, date: cal.date(byAdding: .month, value: -2, to: now).unwrappedOrNow, location: "县城")
    let e7 = Event(name: "新店开业", type: .business, date: cal.date(byAdding: .day, value: 30, to: now).unwrappedOrNow, location: "杭州")
    ...
    let r2 = Record.makeMonetaryRecord(
        ...
        direction: .given,
        paymentMethod: .cash,
        date: cal.date(byAdding: .day, value: -2, to: now).unwrappedOrNow
    )
