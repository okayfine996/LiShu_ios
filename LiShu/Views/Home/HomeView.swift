...
private func makeHomePreviewContainer() -> ModelContainer? {
    ...
    let e1 = Event(name: "表哥的婚礼", type: .wedding, date: cal.date(byAdding: .day, value: 3, to: .now).unwrappedOrNow, location: "北京")
    let e2 = Event(name: "小李的30岁生日", type: .birthday, date: cal.date(byAdding: .day, value: 7, to: .now).unwrappedOrNow, location: "上海")
    let e3 = Event(name: "硕士毕业", type: .education, date: cal.date(byAdding: .day, value: 14, to: .now).unwrappedOrNow, location: "广州")
