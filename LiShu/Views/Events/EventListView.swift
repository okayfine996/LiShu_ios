...
private func makeEventListPreviewContainer() -> ModelContainer? {
    ...
    let e1 = Event(name: "张三的婚礼", type: .wedding, date: cal.date(byAdding: .day, value: 3, to: .now).unwrappedOrNow, location: "北京国贸大酒店")
    let e2 = Event(name: "李四生日宴", type: .birthday, date: cal.date(byAdding: .day, value: 10, to: .now).unwrappedOrNow, location: "上海外滩")
    let e3 = Event(name: "小明毕业典礼", type: .education, date: cal.date(byAdding: .day, value: 21, to: .now).unwrappedOrNow, location: "广州大学")
    let e4 = Event(name: "春节聚会", type: .festival, date: cal.date(byAdding: .month, value: -2, to: .now).unwrappedOrNow, location: "老家")
    let e5 = Event(name: "王五乔迁", type: .property, date: cal.date(byAdding: .month, value: -3, to: .now).unwrappedOrNow, location: "深圳南山")
    ...
    let r1 = Record.makeMonetaryRecord(
        ...
        amount: 500,
        direction: .given,
        date: cal.date(byAdding: .month, value: -2, to: .now).unwrappedOrNow
    )
