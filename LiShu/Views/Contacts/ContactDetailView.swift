...
private func makeContactDetailPreviewContainer() -> (container: ModelContainer, contactID: PersistentIdentifier)? {
    ...
    let weddingEvent = Event(
        name: "参加张三婚礼",
        type: .wedding,
        date: cal.date(from: DateComponents(year: 2025, month: 2, day: 15)).unwrappedOrNow,
        location: "西湖国宾馆"
    )
    ...
    let r2 = Record(
        contact: contact,
        direction: .received,
        date: cal.date(from: DateComponents(year: 2025, month: 1, day: 8)).unwrappedOrNow,
        recordType: .favor
    )
    ...
    let r3 = Record(
        contact: contact,
        direction: .given,
        date: cal.date(from: DateComponents(year: 2024, month: 12, day: 22)).unwrappedOrNow,
        recordType: .gift
    )
    ...
    let r4 = Record(
        contact: contact,
        direction: .given,
        date: cal.date(from: DateComponents(year: 2024, month: 10, day: 5)).unwrappedOrNow,
        recordType: .banquet
    )
    ...
    let birthdayEvent = Event(
        name: "我的生日",
        type: .birthday,
        date: cal.date(from: DateComponents(year: 2024, month: 8, day: 18)).unwrappedOrNow
    )
    ...
    let r6 = Record(
        contact: contact,
        direction: .received,
        date: cal.date(from: DateComponents(year: 2024, month: 6, day: 1)).unwrappedOrNow,
        recordType: .favor
    )
    ...
    let r7 = Record(
        contact: contact,
        direction: .received,
        date: cal.date(from: DateComponents(year: 2024, month: 4, day: 10)).unwrappedOrNow,
        recordType: .gift
    )
